                ...
                <div class="box">
                    <div class="box-header with-border">
                        <div class="pull-right">
                            ...
                            <div class="btn-group pull-right" style="margin-right: 10px">
                                <a href="{{ route('system_admin.services.create') }}" class="btn btn-sm btn-success"
                                    title="New">
                                    ...
                                    @foreach ($services as $service)
                                        <tr>
                                            ...
                                            <td style="width: 100px;">
                                                <img src="{{ $service->image }}" alt="" style="max-width: 100%">
                                            </td>

                                            <td> {!! \App\Helpers\Common::checkStatus($service->status) !!} </td>
                                            <td> {{ \Carbon\Carbon::parse($service->created_at)->format('d/m/Y H:i:s') }}
                                            </td>
                                            <td>
                                                <a href="{{ route('system_admin.services.edit', ['id' => $service->id]) }}"
                    ...
                    <div class="box-footer clearfix">
                        <div class="col-md-5">
                            Hiển thị trang <b>{{ $services->currentPage() }}</b> / <b>{{ $services->lastPage() }}</b>
                        </div>
                        <div class="col-md-7">
                            {{ $services->appends([
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('.deleteDialog').on('click', function() {
                ...
                destroy(post_id, '{{ route('system_admin.services.destroy') }}',
                    '{{ route('system_admin.services.index') }}', "Bạn muốn xóa bài đăng này!");
            });
        });
        $('.grid-batch-0').on('click', function() {
            destroyAll('{{ route('system_admin.services.destroyAll') }}',
                '{{ route('system_admin.services.index') }}', "Bạn muốn xóa các bài đăng đã chọn?");
        });
